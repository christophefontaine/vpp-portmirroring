...
// VLIB_NODE_FUNCTION_MULTIARCH(pm_out_hit_node, pm_out_hit_node_fn)
#else /* DPDK */
...
pm_out_hit_node_fn(vlib_main_t * vm,
    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
    clib_warning("PM out not implemented (no DPDK)");
    return 0;
}
...
static clib_error_t *
pm_out_init(vlib_main_t * vm)
{
    return 0;
}

VLIB_INIT_FUNCTION(pm_out_init);
