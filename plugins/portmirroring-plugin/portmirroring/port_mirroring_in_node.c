    ...
    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
    ...
    while (n_left_from > 0)
    {
        ...
        while (n_left_from > 0 && n_left_to_next > 0)
        {
            ...
            vlib_buffer_t * b0;
            vlib_buffer_t * c0;
            u32 next0 = PM_IN_HIT_NEXT_L2_FWD;
            ...
            if (PREDICT_TRUE(to_int_next != 0))
            {
                ...
                vnet_buffer (c0)->sw_if_index[VLIB_TX] = pm->sw_if_index[0];
...
VLIB_REGISTER_NODE(pm_in_hit_node) = {
    ...
    .next_nodes = {
        [PM_IN_HIT_NEXT_L2_FWD] = "l2-fwd",
    }
};

// VLIB_NODE_FUNCTION_MULTIARCH(pm_in_hit_node, pm_in_hit_node_fn)
#else /* DPDK */
...
pm_in_hit_node_fn(vlib_main_t * vm,
    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
    clib_warning("PM in not implemented (no DPDK)");
    return 0;
}
...
static clib_error_t *
pm_in_init(vlib_main_t * vm)
{
    return 0;
}

VLIB_INIT_FUNCTION(pm_in_init);
